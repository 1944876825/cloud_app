			...
			<view class="" style="width: 750rpx;">
				<video
					...
					<cover-view v-if="isFull && !isLock && (TouchMoveType=='rightYingLiang' || TouchMoveType == 'leftLiangDu')" style="width: 750rpx;position: absolute;top: 60rpx;">
						<view class="" style="display: flex;flex-direction: row;padding: 0 300rpx;">
							<slider
								block-size="0"
								step="5"
								activeColor="#FB7299"
							...
							<scroll-view
								...
								<view class="" 
									...
									<view
										...
										<text v-if="type=='0'" class="" :style="{color: currentJuJi==index ? '#FB7299' : '#fff'}" >{{vod['name']}}</text>
										<text v-if="type=='1'" class="" :style="{color: currentJuJi==index ? '#FB7299' : '#fff'}" >{{vod['fileName']}}</text>
									</view>
								</view>
							</scroll-view>
						</view>
